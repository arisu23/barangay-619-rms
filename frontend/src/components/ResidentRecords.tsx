...
const ResidentRecords: React.FC = () => {
  ...
  const handleSaveResident = (data: any) => {
    ...
    if (
      ...
      setFamiliesData({
        ...familiesData,
        [activeHousehold.id]: updatedFamilies,
      });
      if (selectedFamily?.id === data.householdHeadId) {
        const updatedFam = updatedFamilies.find(
          (f) => f.id === data.householdHeadId,
        );
    ...
    (Object.values(familiesData) as any[][]).forEach((householdFamilies) => {
      householdFamilies.forEach((fam) => {
        ...
        if (memberRecord && memberRecord.role.includes("Head")) {
          ...
          const otherActiveMembers = fam.members.filter(
            (m: any) => m.id !== resident.id && m.status === "Active",
          );
  ...
  const handleConfirmArchive = () => {
    if (residentToArchive && archiveStatus) {
      ...
      if (selectedFamily) {
        ...
        const refreshedFam = (Object.values(updatedFamiliesData) as any[][])
          .flat()
          .find((f: any) => f.id === selectedFamily.id);
  ...
  const handleViewFamilyDetail = (family: any) => {
    ...
    const activeMembers = updatedFamily.members.filter(
  ...
  const handleSetNewHead = (memberId: number) => {
    ...
    const targetMember = selectedFamily.members.find(
      (m: any) => m.id === memberId,
    );
    ...
    const newHeadName =
      updatedMembers.find((m: any) => m.id === memberId)?.name || "";
    ...
    const updatedHouseholdFamilies = familiesData[activeHousehold.id].map(
      (fam) => (fam.id === selectedFamily.id ? updatedFamily : fam),
    );
    setFamiliesData({
      ...familiesData,
      [activeHousehold.id]: updatedHouseholdFamilies,
    });
  ...
  return (
    ...
    >
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        color={
                          resident.status === "Active" ? "success" : "default"
                        }
                        sx={{ fontWeight: 700 }}
                      />
                    </TableCell>
      ...
      >
        {activeHousehold &&
          (familiesData[activeHousehold.id] as any[])?.map((family) => (
            <MenuItem
              key={family.id}
              onClick={() => handleViewFamilyDetail(family)}
              sx={{ py: 1.5 }}
            >
              {family.head} (
              {family.members.filter((m: any) => m.status === "Active").length}{" "}
              active)
            </MenuItem>
          ))}
      </Menu>
      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 800 }}>
              Family Detail: {selectedFamily?.head}
            </Typography>
          </Box>
          <Stack direction="row" spacing={1}>
            <Button
              ...
              onClick={() => handleOpenAddModal(selectedFamily?.id)}
              sx={{
                bgcolor: "#2e0249",
                textTransform: "none",
                borderRadius: 2,
        ...
        <DialogContent sx={{ p: 0 }}>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        sx={{
                          ...
                          fontSize: "0.7rem",
                          bgcolor:
                            m.role === "Head of Family" ? "#fef3c7" : "#f3f4f6",
                          color:
                            m.role === "Head of Family" ? "#92400e" : "#64748b",
                        }}
                      />
                    </TableCell>
          ...
          {selectedFamily?.members.filter((m: any) => m.status === "Active")
            .length > 1 && (
            <Box
            ...
            >
              ...
              >
                <Info size={14} /> Only active members can be designated as the
                Head of Family.
              </Typography>
            </Box>
          )}
