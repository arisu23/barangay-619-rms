...
export class OfficialController {
  ...
  //Get active officials (effective today)
  static async getActiveOfficials(
    ...
    try {
      const officials = await OfficialService.getActiveOfficials();
      res.json({ success: true, data: officials });
    } catch (error) {
      next(error);
    }
  ...
  //Add official
  static async addOfficial(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  //Update official
  static async updateOfficial(req: Request, res: Response, next: NextFunction) {
    try {
      const officialId = Number(req.params.id);
      const userId = req.user!.userId;
