...
export class FamilyController {
  ...
  static async assignHouseholdHead(req: Request, res: Response) {
    try {
      ...
      await FamilyService.assignHouseholdHead(
        Number(householdId),
        Number(residentId),
        userId,
      );
  ...
  static async changeHouseholdHead(req: Request, res: Response) {
    try {
      ...
      await FamilyService.changeHouseholdHead(
        Number(householdId),
        Number(currentHeadId),
        userId,
      );
  ...
  static async getPrimaryHeads(_req: Request, res: Response) {
    try {
      ...
      res.status(err.status || 500).json({
        success: false,
        message: err.message || "Failed to get family heads!",
      });
    }
  }

  //Demote a primary family head
  static async demoteHouseholdHead(req: Request, res: Response) {
    try {
      const { residentId } = req.body;
      const userId = req.user!.userId;

      await FamilyService.demoteHouseholdHead(Number(residentId), userId);

      res.status(200).json({
        success: true,
        message: "Family head demoted successfully!",
      });
    } catch (err: any) {
      res.status(err.status || 500).json({
        success: false,
        message: err.message || "Failed to demote family head!",
      });
    }
  }
}
